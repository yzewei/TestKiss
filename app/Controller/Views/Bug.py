...
import xlrd
from app import app
from app.Models.BugModel import Bug
...
def dashboard():
        ...
        return render_template('bug/choose_pd.html', products=products, url=request.path, params=params)


@bug.route('/lists', methods=['GET'])
@login_required
def lists():
        r = request.args
        ...
        return render_template('bug/bug.html', product=product, modules=modules)


@bug.route('/edits', methods=['GET'])
@login_required
def edit_bugs():
        r = request.args
        product_id = r.get('product_id', '')
        ...
        return render_template('bug/edit.bug.html', product=product, modules=modules)


@bug.route('/delete', methods=['DELETE'])
@login_required
def delete_bug():
    r = request.form
    cs = Bug()
...
@bug.route('/quick/save', methods=['PUT'])
@login_required
def quick_save():
    r = request.form
    _id = r.get('_id', '')
...
@bug.route('/edit', methods=['GET', 'PUT'])
@login_required
def edit_one():
    cs = Bug()
    if request.method == "GET":
        ...
        bug_ = cs.table.find_one({"_id": int(_id)})
        if bug_ is None:
            flash("bug 不存在！")
            return redirect(request.referrer)

        return render_template('bug/edit.one.html', cs=bug_, ref=request.referrer)
