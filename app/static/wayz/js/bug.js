...
function add_bug_node(li_id, bug_id, bug_name) {
    let li = $("#li_"+li_id)
    let html = "<li id=\"bug_"+bug_id+"\" class=\"bug-node draggable\" style=\"display: list-item;\">\n" +
        "<span>"+bug_name+"</span>\n" +
        "</li>"
...
function delete_bug(bug_id) {
    let url = current_host + '/bug/delete'
    let data = "_id="+ bug_id
        $.ajax({
        type : "DELETE",
        url : url,
...
function sort_bug(bug_id, md, st) {
    let url = current_host + '/bug/sort'
    let data = "_id="+ bug_id+ "&st="+st+"&md="+md
        $.ajax({
        type : "PUT",
        url : url,
        dataType: "json",
...
function change_order(){
    ...
    $(".bug-node").draggable({
        ...
        stop: function (event, ui) {
            ...
            let bug_id = $(this).attr('id').split("_")[1]
            let st = ""
            if($(this).parent('ul').hasClass('delete-node')){
                console.log("删除用例:" + bug_id)
                delete_bug(bug_id)
                $(this).hide()
                return
            }
            ...
            $.ajax({
                ...
                success : function(r){
                    ...
                    if(r.err == 0){
                        ...
                        msg("success", r.msg)
                        add_bug_node(module, r.data._id, name)
                        $("#success_msg").text("")
                        $("#bug_name").val('')
                        $("#editor-one").empty()
                        $("#editor-two").empty()
...
function upload_template(){
    ...
    $("#confirm").click(function () {
        let url = current_host + "/bug/batch"
        let name = "file";
        let product_id = $("#product_id").val();
...
function show_bug_num(){
    let parents = $(".parent_li")
    $.each(parents, function (i) {
        let p = $(parents[i]).find('.bug-node').length
